...
#include "imguitexteditor/imguitexteditor.h"
...
int TextEditor::InsertTextAt(Coordinates& aWhere, const char* aValue, bool aRedo)
{
    assert(!mReadOnly);
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
    ...
    ImVec4 bgCol = ImGui::ColorConvertU32ToFloat4(mPalette[(int)PaletteIndex::Background]);
    bgCol.w = 0.25f;
    ...
    for (auto chr : aText)
    {
        ...
        else if (chr == '\t')
        {
            for (int i=0; i<mTabSize; i++)
                mLines.back().emplace_back(Glyph(' ', PaletteIndex::Default));
        }
        else
    ...
    else
    {
        ...
        for (size_t i = 0; i < aLines.size(); ++i)
        {
            ...
            for (size_t j = 0; j < aLine.size(); ++j)
            {
                ...
                if (c == '\t')
                {
                    for (int k=0; k<mTabSize; k++)
                        mLines[i].emplace_back(Glyph(' ', PaletteIndex::Default));
                }
                else
                    mLines[i].emplace_back(Glyph(c, PaletteIndex::Default));
            }
        }
    }
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift, bool aAddUndo)
{
    ...
    if (HasSelection())
    {
        if (aChar == '\t' && mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine)
        {
            ...
            if (!shift)
            {
                ...
                {
                    UndoRecord uu;
                    uu.mAddedStart.mLine = i;
                    uu.mAddedStart.mColumn = 0;
                    uu.mPropagate = i > start.mLine;
                    uu.mBefore = mState;
                    std::string spaces(mTabSize, ' ');
                    auto lineStart = Coordinates(i, 0);
                    InsertTextAt(lineStart, spaces.c_str());
                    uu.mAdded = spaces;
                    uu.mAddedEnd.mLine = i;
                    uu.mAddedEnd.mColumn = mTabSize;
                    uu.mAfter = mState;
                    AddUndo(uu);
                }
                mState.mSelectionStart = Coordinates(start.mLine, start.mColumn+4);
                mState.mSelectionEnd = Coordinates(end.mLine, end.mColumn+4);
            }
            else
            {
                ...
                for (int i = start.mLine; i <= end.mLine; i++)
                {
                    ...
                    auto& line = mLines[i];
                    int maxStep = std::min((int)line.size(), mTabSize);
                    while(nRemovedSpaces<maxStep)
                    {
                        if (line[nRemovedSpaces].mChar != ' ')
                            break;
                        nRemovedSpaces++;
                    }
                    if (i == start.mLine)
                    ...
                    UndoRecord uu;
                    uu.mRemoved = spaces;
                    uu.mRemovedStart.mLine = i;
                    uu.mRemovedStart.mColumn = 0;
                    uu.mPropagate = i > start.mLine;
                    uu.mBefore = mState;
                    auto lineStart = Coordinates(i, 0);
                    auto lineEnd = Coordinates(i, nRemovedSpaces);
                    DeleteRange(lineStart, lineEnd);
                    uu.mRemovedEnd.mLine = i;
                    uu.mRemovedEnd.mColumn = nRemovedSpaces;
                    uu.mAfter = mState;
                    AddUndo(uu);
                }
                mState.mSelectionStart = Coordinates(start.mLine, start.mColumn-deltaStartCol);
                mState.mSelectionEnd = Coordinates(end.mLine, end.mColumn-deltaEndCol);
            }
    ...
    else
    {
        if (aChar == '\t')
        {
            ...
            int n = mTabSize-coord.mColumn%mTabSize;
            u.mPropagate = false;
            u.mBefore = mState;
            for (int i=0;i<n;i++)
                EnterCharacter(' ', aShift, false);
            u.mAdded = std::string(n, ' ');
...
void TextEditor::InsertText(const std::string & aValue, bool aRegisterUndo, bool aPropagateUndo)
{
    InsertText(aValue.c_str(), aRegisterUndo, aPropagateUndo);
}

void TextEditor::InsertText(const char * aValue, bool aRegisterUndo, bool aPropagateUndo)
{
    if (aValue == nullptr)
        return;
    ...
    if (aRegisterUndo)
    {
        u.mPropagate = aPropagateUndo;
        u.mAdded = aValueNoTabs;
...
void TextEditor::Delete(bool aPropagateUndo)
{
    assert(!mReadOnly);
...
void TextEditor::Undo(int aSteps)
{
    ...
    {
        auto& u(mUndoBuffer[mUndoIndex-1]);
        if (mUndoBuffer[mUndoIndex-1].mPropagate)
            aSteps++;
...
void TextEditor::Redo(int aSteps)
{
    ...
    {
        if (mUndoIndex+1 < mUndoBuffer.size())
        {
            if (mUndoBuffer[mUndoIndex+1].mPropagate)
                aSteps++;
        }
...
void TextEditor::EnsureCursorVisible()
{
    ...
    // Edited by virmodoetiae to have a personally more desirable horizontal 
    // scrolling behaviour
    if (pos.mLine < top)
    ...
    if (pos.mColumn < left)
        ImGui::SetScrollX(std::max(0.0f, (pos.mColumn - 1) * mCharAdvance.x));
    if (pos.mColumn > right - 8)
        ImGui::SetScrollX(std::max(0.0f, (pos.mColumn + 8) * mCharAdvance.x - width));
}
...
void TextEditor::UndoRecord::Redo(TextEditor * aEditor)
{
    ...
    if (!mAdded.empty())
    {
        ...
        aEditor->InsertTextAt(start, mAdded.c_str(), true);
