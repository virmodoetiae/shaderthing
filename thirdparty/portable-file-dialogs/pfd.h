...
//
// Edit by virmodoetiae <sradman@protonmail.com> - 26/02/2024
//
// You have to define the macro PFD_IMPLEMENTATION in a single source file 
// before including this header file, e.g.,
//
// #define PFD_IMPLEMENTATION
// #include "thirdparty/portable-file-dialogs/pfd.h"
//
// in order to include the actual implementation. This is currently done
// in pfd_impl.cpp, so for the present project structure you need not worry
// about this
//
...
//
// Below this are all the method implementations. You have to define the
// macro PFD_IMPLEMENTATION in a single source file before including this
// header
//

#if defined PFD_IMPLEMENTATION

// internal free functions implementations

opt operator |(opt a, opt b) { return opt(uint8_t(a) | uint8_t(b)); }
bool operator &(opt a, opt b) { return bool(uint8_t(a) & uint8_t(b)); }
...
// Currently broken for open/save file dialogs, does nothing but wait for a user
// selection (only tested on Windows 10 and 11 for now) - virmodoetiae
